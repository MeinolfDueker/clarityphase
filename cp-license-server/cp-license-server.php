...
function cp_lic_register_cpt() {
    register_post_type(CP_LIC_CPT, [
        'label'           => esc_html__('Lizenzen', 'cp-license-server'),
        'public'          => false,
        'show_ui'         => true,
...
add_action('add_meta_boxes', function() {
    add_meta_box('cp_lic_box', esc_html__('Lizenzdaten', 'cp-license-server'), 'cp_lic_render_metabox', CP_LIC_CPT, 'normal', 'high');
});

function cp_lic_render_metabox($post) {
    if (!current_user_can('manage_options')) {
        echo '<p>' . esc_html__('Keine Berechtigung.', 'cp-license-server') . '</p>';
        return;
    }
    ...
    echo '<tr><th scope="row"><label for="cp_license_key">' . esc_html__('Lizenzschlüssel', 'cp-license-server') . '</label></th><td>';
    ...
    echo '<tr><th scope="row"><label for="cp_plan">' . esc_html__('Plan', 'cp-license-server') . '</label></th><td>';
    ...
    echo '<tr><th scope="row"><label for="cp_status">' . esc_html__('Status', 'cp-license-server') . '</label></th><td>';
    ...
    echo '<tr><th scope="row"><label for="cp_expires">' . esc_html__('Ablaufdatum', 'cp-license-server') . '</label></th><td>';
    ...
    echo '<tr><th scope="row"><label for="cp_max_domains">' . esc_html__('Max. Domains', 'cp-license-server') . '</label></th><td>';
    ...
    echo '<tr><th scope="row"><label for="cp_domains">' . esc_html__('Gebundene Domains', 'cp-license-server') . '</label></th><td>';
...
function cp_lic_render_secret_page() {
    ...
    if (isset($_POST['cp_secret_nonce']) && wp_verify_nonce($_POST['cp_secret_nonce'], 'cp_secret_save')) {
        ...
        echo '<div class="notice notice-success"><p>' . esc_html__('Secret gespeichert.', 'cp-license-server') . '</p></div>';
    }

    $secret = (string) get_option(CP_LIC_API_SECRET_OPT, '');
    echo '<div class="wrap"><h1>' . esc_html__('CP License API Secret', 'cp-license-server') . '</h1>';
    ...
    echo '<button class="button button-primary">' . esc_html__('Speichern', 'cp-license-server') . '</button>';
    echo '</form>';
